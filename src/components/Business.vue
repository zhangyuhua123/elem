    ...
    <section class="business_content"
      ...
      <ul class="business_left" ref="businessLeft">
        <li v-for="(item, index) in business_info.commodity"
            :key="index" @click="leftControlRightScroll(index)"
            :class='{active_ia:index == 0}'>
          {{ item.name }}
          <span
                class="left_red"
                v-if="reNub[item.type_accumulation]"
          >
            ...
            <div class="single_r">
              ...
              <div class="add_remove">
                ...
                <div class="add_remove_btn">
                  <svg @click="reduce_food(x.one_food_id)"
                  ...
                  <span class="commodity_count"
                        v-if="shoppingCarList[x.one_food_id]">
                    {{ shoppingCarList[x.one_food_id].count }}
                  </span>
...
export default {
  ...
  watch: { //  watch对于对象来说deep=true也只能监控对象原有属性，新增加的无效！！！！
